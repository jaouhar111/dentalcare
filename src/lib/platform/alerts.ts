import { EventStatus, SubscriptionStatus } from "@prisma/client";
import { db } from "@/lib/db/client";
import type { PlatformAlert } from "@/server/actions/super-admin-alerts-types";

/**
 * Derives the platform's active alerts (health + billing) from live data.
 * Pure read, NO auth check — callers gate access (the super-admin action
 * requires SUPER_ADMIN; the Inngest digest cron runs trusted). Shared so
 * the dashboard panel and the email digest never drift.
 */
